...
describe("Schema Unit Tests", () => {
  describe("DB Schema", () => {
    ...
    describe("DB_DATABASE", () => {
      test('invalid cases', () => {
        expectValidate(schema, { DB_DATABASE: 'sqlite' }).not.toContain('"DB_DATABASE" is required')

        expectValidate(schema, { DB_DATABASE: 'mysql' }).not.toContain('"DB_DATABASE" is required')
    ...
    describe("DB_USERNAME", () => {
      test('invalid cases', () => {
        expectValidate(schema, { DB_USERNAME: 'sqlite' }).not.toContain('"DB_USERNAME" is required')

        expectValidate(schema, { DB_USERNAME: 'mysql' }).not.toContain('"DB_USERNAME" is required')

        expectValidate(schema, { DB_USERNAME: 1 }).toContain('"DB_USERNAME" must be a string')
      })
    ...
    describe("DB_PASSWORD", () => {
      test('invalid cases', () => {
        expectValidate(schema, { DB_PASSWORD: 'sqlite' }).not.toContain('"DB_PASSWORD" is required')

        expectValidate(schema, { DB_PASSWORD: 'mysql' }).not.toContain('"DB_PASSWORD" is required')

        expectValidate(schema, { DB_PASSWORD: 1 }).toContain('"DB_PASSWORD" must be a string')
      })
    ...
    describe("DB_PORT", () => {
      test('invalid cases', () => {
        expectValidate(schema, { DB_PORT: 'sqlite' }).not.toContain('"DB_PORT" is required')

        expectValidate(schema, { DB_PORT: 'mysql' }).not.toContain('"DB_PORT" is required')

        expectValidate(schema, { DB_PORT: 'a' }).toContain('"DB_PORT" must be a number')

        expectValidate(schema, { DB_PORT: 1.2 }).toContain('"DB_PORT" must be an integer')
